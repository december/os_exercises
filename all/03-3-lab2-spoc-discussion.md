...
> 前三个需要。这里假定ALU完成计算所需数据都已经在CPU内部了，其他操作均需要检查访问权限。
...
```
 ...
 ```
- 首先初始化GDT，将CR0的PE位置1，开启保护模式，同时使能段机制。接下来进行物理内存探测，初始化可用内存，依次进行页初始化、计算起始地址、检查内存管理算法等。完成上述过程后，建立二级页表，将CR0的最后一位置1，使能页机制。  
...
- 我的学号是2012011359，mod 37得14，加入以上语句后make qemu后不再按时产生100 tick的时钟中断信息，而是打印了以下内容：
...
－ 该汇编语句的含义是触发中断号为14的中断，将由trap.c中的tran_dispatch()函数中的switch结构处理。该switch结构按中断号列出了几种中断的处理方式，如时钟中断、串口、键盘等，而中断号14不在其列，因此将导致default语句的执行，即unexpected trap in kernel，触发异常。
...
- 程序代码在[这里](https://github.com/december/os_data/blob/master/translation.py)查看，运行结果如下：  
