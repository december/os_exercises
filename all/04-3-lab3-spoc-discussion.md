...
若要实现clock算法，需要额外维护一个环形链表保存访问位，并增加一个从起点开始逐一访问链表各结点、找到第一个访问位为0的结点并将经过的访问位为1的结点置零的函数，在swap_out_victim和map_swappable的时候调用这个函数来决定置换的页面，其余与现有代码相同。

由于clock算法只记录页是否被访问过，而不能体现页被访问的频繁次数。因此当某一页或几页被连续访问多次，之后再访问其它页面，这些页面很容易因为时间差过长而被替换掉，这样性能就会与LRU算法有很大差距。

不太可能实现，LRU算法需要CPU支持在访问内存时记录访问时间。而Lab中的CPU仅能在访问时对标记位进行标记。
